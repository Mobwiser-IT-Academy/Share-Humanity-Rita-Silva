...
//   for (issues) {
//     if (!filterByCategory || filterByCategory === issue.category) {
//       createHtml += ...
//     }
//   }
...
function createHtml(filterByCategory) {
  ...
  for (let i = 0; i < issues.length; i++) {
    ...
    if (!filterByCategory || issues.category.some((issues) => issues === filterByCategory)) {
...
//     issuesHtml += `<div id=${array[i].class}><img src=${array[i].src}></img><h5>${array[i].title}</h5> <p>${array[i].description}</p> <button> <a href=${array[i].href}>View Detail</a></button></div>`
//   }
