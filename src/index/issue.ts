...
createHtml(filterByCategory: string):void{
    ...
    for (let i = 0; i < this.issues.length; i++) {
        if (
            !filterByCategory ||
      this.issues[i].category.some((issue) => issue === filterByCategory)
        ) {
            issuesHtml += `<div><img src=${this.issues[i].src}></img><h5>${this.issues[i].title}</h5> <p>${this.issues[i].description}</p> <button> <a href=${this.issues[i].href}>View Detail</a></button></div>`;
        }
    }
